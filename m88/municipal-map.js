...
function primaryPolygon(feature){
  ...
  return polygons.reduce((best,polygon)=>ringArea(polygon?.[0])>ringArea(best?.[0])?polygon:best,polygons[0]);
}
...
function islandGeoBounds(features){
  ...
  for(const feature of features){
    const polygon = primaryPolygon(feature);
    if(polygon?.[0]) points.push(...polygon[0]);
  }
...
function geometryPath(geometry,geoBounds,targetBounds){
...
function ensureStyles(){
  ...
  style.textContent=`
    ...
    @media(max-width:760px){.municipality-shape{stroke-width:.42}.municipality-shape:hover{stroke-width:.75}.pipeline-legend{width:90vw;margin-top:12px;gap:7px 12px;font-size:7px}.pipeline-legend-title{width:100%;text-align:center;margin:0 0 2px}.municipality-tooltip{display:none!important}}
...
function renderMunicipalityOverlay(svg,stage,features,statusMap){
  ...
  for(const island of islands.filter(item=>item.municipalities.length)){
    ...
    group.dataset.island=island.slug;
